...
namespace ZiraLink.Api.Application.Services
{
    public interface IAppProjectService
    {
        ...
        Task<long> CreateAsync(long customerId, string title, string viewId, AppProjectType appProjectType, int internalPort, RowState state, CancellationToken cancellationToken);
        Task DeleteAsync(long customerId, long id, CancellationToken cancellationToken);
        Task PatchAsync(long id, long customerId, string title, string viewId, AppProjectType appProjectType, int internalPort, RowState state, CancellationToken cancellationToken);
    }
}
